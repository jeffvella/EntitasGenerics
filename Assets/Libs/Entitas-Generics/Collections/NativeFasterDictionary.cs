...
{
    // https://github.com/jeffvella/NativeFasterDictionary
    ...
    public unsafe struct NativeFasterDictionary<TKey, TValue> : IDisposable where TKey : struct, IComparable<TKey> where TValue : struct
    {
        ...
        public T* CastPtr<T>(void* ptr, int offset = 0) where T : unmanaged
        {
        ...
        public void Expand(int newSize)
        {
            ...
            // issue: things still reading from ptrs/refs after Free is called crashes unity
        ...
        public void* GetValuePointer(int index)
        {
            return Data->Values.AsPtr(index);
        }
        ...
        public bool TryFindIndex(TKey key, out int findIndex)
        {
            ...
            //for (int i = 0; i < Data->Nodes.Length; i++)
            var maxNodeIndex = Data->FreeValueCellIndex;
        ...
        internal sealed class NativeFasterDictionaryDebugView
        {
            ...
            public Node[] Nodes => _source.Data->Nodes.ToArray<Node>();
