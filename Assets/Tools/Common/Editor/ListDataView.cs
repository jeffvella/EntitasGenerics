...
public abstract class ListDataView<T> where T : class, new()
{
    ...
    {
        _list = new ReorderableList(Data, typeof(ComboDefinition), true, false, true, true)
        {
            ...
            drawElementCallback = (rect, index, active, focused) => { GUI.Label(rect, GetElementName(Data[index])); }
        };
    }
